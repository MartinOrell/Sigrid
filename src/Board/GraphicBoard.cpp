...
void GraphicBoard::addBorder(){
    ...
    moveTiles({dx, dy});
    if(m_labelsPtr){
        m_labelsPtr->moveLeftInsideLabels({dx, dy});
        m_labelsPtr->moveBottomInsideLabels({dx, dy});
        m_labelsPtr->moveLeftOutsideLabels({0.f, dy});
        m_labelsPtr->moveBottomOutsideLabels({dx, 2.f*dy});
    }
